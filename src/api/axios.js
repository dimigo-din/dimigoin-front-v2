...
//       error.config.headers.Authorization = `Bearer ${store.state.account.auth.accessToken}`;
...
//       await store.dispatch('account/regenerateAccessToken');
//       // The above dispatch patches default headers, but not in this request.
//       config.headers.Authorization = `Bearer ${store.state.account.auth.accessToken}`;
//     } catch (err) {
//       await store.dispatch('account/logout');
//       throw err;
//     }
